...
describe('TaskCompletionPartView', () => {
  ...
  it('renders description as headline over the raw command', () => {
    const withDesc: TaskCompletionPart = {
      ...part,
    ...
    render(<TaskCompletionPartView part={withDesc} />)

    const toggle = screen.getByRole('button', { name: /Background task completed/i })
    // description 主显，command 折进展开体
    expect(toggle.textContent).toContain('Build the backend')
    expect(toggle.textContent).not.toContain('sleep 10')
