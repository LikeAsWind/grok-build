...
export const InlinePermission = memo(function InlinePermission({
  ...
  // ACP 下发的动态选项（如 allow-always-command"总是允许 git push"）。
  // 有则按 kind 排序渲染并回显 optionId；无（REST 旧路径）回退 3 个语义按钮
      ...
      {resolved ? (
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {acpOptions.map(opt => (
            <button
              key={opt.id}
              onClick={() => onReply(request.id, { optionId: opt.id })}
              disabled={isReplying}
...
/** kind → 按钮样式（primary 主按钮实底,其余弱化;danger 悬停红） */
const OPTION_TONE_CLASS: Record<ReturnType<typeof permissionOptionTone>, string> = {
  primary: 'font-medium bg-text-100 text-bg-000 hover:bg-text-200',
  secondary: 'text-text-300 hover:text-text-100',
  muted: 'text-text-400 hover:text-danger-100',
  danger: 'text-danger-100/70 hover:text-danger-100',
}
